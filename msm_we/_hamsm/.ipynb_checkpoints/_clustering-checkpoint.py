...
class ClusteringMixin:
    ...
    def do_stratified_clustering(self: "modelWE", arg):
        ...
        max_iter_for_clustering_loop = (np.max(iters_to_use))+1  #SagarComments:Included
        # Cast to a list in case it's a np array
        iters_to_use = list(iters_to_use)
        ...
        iteration = -1
        # Maybe not even necessary to track iter_coords, just _iter_coords.
        # SagarComments: Collect all data for all WE iteration at once to remove the step of nearest fill bin.  
        while not ((iteration+1) >= (max_iter_for_clustering_loop)):
            unfilled_bins = []
            ...
            if used_iters == 0:
                ...
                pcoords = [x for x in self.pcoord1List]
            else:
                iter_coords = np.append(iter_coords, _iter_coords, axis=0)
                pcoords.extend(self.pcoord1List)
                if self.use_weights_in_clustering:
                    seg_weights = np.append(seg_weights, _seg_weights, axis=0)
                log.debug(f"After extension, pcoords: {len(pcoords)}, iter_coords: {iter_coords.shape}. Ignored: {ignored_bins}. Mapper: {bin_mapper}")
        
        # SagarComments: Map coords to WE bins. This is done to check that every WE bins have segments this section of code remaining
        ...
        if len(pcoord_array) > 0:
            ...
            log.debug("No coordinates outside of basis/target, we_bin_assignments will be empty and clustering"f" will be skipped for this iteration. ({sum(pcoord_is_target)} in target, {sum(pcoord_is_basis)} in basis)")
        ...
        if not all_bins_have_segments:
            print(f"couldn't get segments in all bins, and no iterations left. Increase Strata size or reduce clusters per strata")
            #SagarTODOS: Which bin didn't have enough segs? For those bins use nearest filled bin
            #SagarTODOs: Will include later
        
        # SagarComments:Included to remove pcoord_is_target_or_basis 
        pcoord_array = np.array(pcoords)
        pcoord_is_target = self.is_WE_target(pcoord_array)
        pcoord_is_basis = self.is_WE_basis(pcoord_array)
        pcoord_is_target_or_basis = np.where((pcoord_is_target == True) | (pcoord_is_basis == True))
        if (len(pcoord_is_target_or_basis[0])>0):
            iter_coords = np.delete(iter_coords, pcoord_is_target_or_basis[0], axis=0)
            pcoord_array = np.delete(pcoord_array, pcoord_is_target_or_basis[0], axis=0)
            if self.use_weights_in_clustering:
                seg_weights = np.delete(seg_weights, pcoord_is_target_or_basis[0], axis=0)
        if len(pcoord_array) > 0:
            we_bin_assignments = bin_mapper.assign(pcoord_array)
        else:
            we_bin_assignments = np.array([])
    ...
    def update_cluster_structures(self: "modelWE", build_pcoord_cache=False):
        ...
        # This can be used to map a structure in a cluster to its west.h5/iteration/segment_id
        structure_iteration_segments = dict()
        ...
        self.structure_iteration_segments = structure_iteration_segments
    ...
    def get_cluster_centers(self: "modelWE"):
        ...
        #SagarComments:The above line commented to check numpy3.10version compatible.
